...
pub fn generate_project(
    ...
    let runtime_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("simulator-rust-runtime");
    let runtime_path = relative_path(&absolute_output, &runtime_dir)?;
    let manifest = format!(
        "[package]\nname = {:?}\nversion = \"0.1.0\"\nedition = \"2024\"\npublish = false\n\n[dependencies]\nruint = {{ version = \"1.20\", default-features = false }}\nsimulator-rust-runtime = {{ path = {:?} }}\n\n[workspace]\n",
        options.crate_name,
...
impl<'a> Generator<'a> {
    ...
    fn generate(mut self) -> Result<GeneratedSources, GenerateError> {
        ...
        writeln!(
            lib_rs,
            "//! Standalone two-state simulator generated by simulator-rust.\n"
        )
        .unwrap();
        writeln!(
            lib_rs,
        ...
        writeln!(
            lib_rs,
            "use simulator_rust_runtime::{{BitSerialize, Bits, array_offset, bool_and, bool_or, bool_xor}};\n"
        )
        .unwrap();

        let mut types_rs = String::from(
            "//! Generated RTL data types.\n\nuse simulator_rust_runtime::{BitSerialize, Bits, array_offset};\n\n",
        );
